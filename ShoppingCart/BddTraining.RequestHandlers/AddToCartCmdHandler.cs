...
namespace BddTraining.RequestHandlers
{
    public class AddToCartCmdHandler: IAddToCartCmdHandler
    {
        ...
        public AddToCartCmdHandler(
            IShoppingCartRepository cartRepository, 
            IProductRepository productRepository, 
            IShoppingCartRetriever cartRetriever)
        {
        ...
        {
            var shoppingCart = _cartRetriever.TryGet(command.CartId);

            var product = _productRepository.Get(command.ProductId);

            shoppingCart.Add(product, command.Quantity);
